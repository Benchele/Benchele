...
int atiendeOrdenes(struct Orden *ordenes, int n, struct Linea *lineas, int m, double alfa) {
...
int problema(double alfa) {
    ...
    int mejorMakeSpan = atiendeOrdenes(ordenes, n, lineas, m, alfa);
...
double buscarPromedio(double alfa) {
    ...
    for (int k = 0; k < 20; k++) {
        cantidadPorIteracion += problema(alfa);
    }
