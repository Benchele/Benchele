...
void imprimir(struct Linea *lineas, struct Orden *ordenes, int M) {
    ...
    for (int i = 0; i < M; i++) {
        ...
        if (makespan < lineas[i].cargaTotal) {
...
int procesarOrdenes(struct Orden *ordenes, int N, struct Linea *lineas, int M, double tiempoPorBotella) {
    ...
    for (int k = 0; k < N; k++) {
        //Ordenamos para que la linea que tenga menos carga esté al inicio -> lineas[0]
        sort(lineas, lineas + M, compara);

        double cargaOrden = ordenes[k].cantidad * tiempoPorBotella; // Calcular carga de esta orden

        // Asignar la orden a la línea con menor carga
...
int main() {
    ...
    /*
     Makespan (Tiempo acumulado de trabajo):
        Se refiere al tiempo total que tarda en completarse un conjunto de tareas,
        desde que se inicia la primera hasta que termina la última.
    */
