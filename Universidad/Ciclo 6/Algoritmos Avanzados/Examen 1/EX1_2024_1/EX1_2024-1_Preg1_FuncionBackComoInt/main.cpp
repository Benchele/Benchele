...
int encontrarCamino(int mapa[][N], int x, int y, int costoActual, int &costoMinimo) {
    if (x == N - 1 and y == N - 1) {
    ...
    for (int g = 0; g < MAX_MOV; ++g) {
        ...
        if (esValido(mapa, mx, my)) {
            ...
            //if (encontrarCamino(mapa, mx, my, costoActual, costoMinimo)) { // ... *
            if (encontrarCamino(mapa, mx, my, costoActual + mapa[mx][my], costoMinimo)) {
                return 1;
            }
