...
//Calcular fitness
int calculaFitness(vector<int> cromosoma, int *valores) {
    int sumaValores = 0;

    for (int k = 0; k < cromosoma.size(); k++) {
        if (cromosoma[k] == 1) {
            sumaValores += valores[k];
        }

        //O puedes hacerlo asi
        //sumaPeso += valores[k] * cromosoma[k];
    }
...
//Calculo peso
int calculaPeso(vector<int> cromosoma, int *paquete) {
    int sumaPeso = 0;
...
int aberracion(vector<int> individuo, int *paquetes, int peso) {
    return calculaPeso(individuo, paquetes) > peso;
}
...
void calculaSupervivencia(vector<vector<int>> poblacion, vector<int> &supervivencia, int *valores) {
    ...
    for (int k = 0; k < poblacion.size(); k++) {
        sumaFitness += calculaFitness(poblacion[k], valores); //Sumatoria de fitness - 100%
    }

    for (int k = 0; k < poblacion.size(); k++) {
        int superFit; // Dividir cada fitness entre el total
        superFit = round(100 * (double)calculaFitness(poblacion[k], valores)/sumaFitness); // Porcentaje de superviviencia
...
void seleccion(vector<vector<int>> poblacion, vector<vector<int>> &padres, int *valores) {
...
//////////////////////////////////////////////
void regenerarPoblacion(vector<vector<int>> &poblacion, int *valores) {
    mataClon(poblacion);

    sort(poblacion.begin(), poblacion.end(),
        [valores](const vector<int> &k, const vector<int> &g) { //Funcion lambda
            return calculaFitness(k, valores) > calculaFitness(g, valores);
        }
    );
...
//////////////////////////////////////////////
int muestarMejor(vector<vector<int>> poblacion, int *paquetes, int *valores, int peso) {
    int mejor = 0;

    //Se puede asumir que la mejor poblacion esta en la posicion 0
    for (int k = 0; k < poblacion.size(); k++) {
        if (calculaFitness(poblacion[mejor], valores) < calculaFitness(poblacion[k], valores)) {
            mejor = k;
        }
    }

    cout << endl << "=== MEJOR SOLUCION ACTUAL ===" << endl;
    cout << "Valor total: " << calculaFitness(poblacion[mejor], valores) << endl;
    cout << "Peso total: " << calculaPeso(poblacion[mejor], paquetes) << endl;
    ...
    return peso - calculaPeso(poblacion[mejor], paquetes);
}
...
//////////////////////////////////////////////
//Principal
void mochilaGenetica(int *paquetes, int n, int peso, int *valores) {
...
int main() {
    ...
    int valores[] = {10, 5, 20, 15, 25, 30, 35, 40};
    ...
    mochilaGenetica(paquetes, n, peso, valores);
